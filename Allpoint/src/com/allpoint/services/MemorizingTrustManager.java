...
public class MemorizingTrustManager implements X509TrustManager {
	...
	private String certificateToTrust;
	
	private X509Certificate certificateToMatch;
	...
	public MemorizingTrustManager(Context m, String trustedCertificate) {
		init(m);
		this.appTrustManager = getTrustManager(appKeyStore);
		this.defaultTrustManager = getTrustManager(null);
		this.certificateToTrust = trustedCertificate;
	}
	
	public MemorizingTrustManager(Context m, X509Certificate certificateToMatch) {
		init(m);
		this.appTrustManager = getTrustManager(appKeyStore);
		this.defaultTrustManager = getTrustManager(null);
	...
	public void deleteCertificate(String alias) throws KeyStoreException {
		System.out.println("#info deleteCertificate - " + alias);
	...
	{		
		if(certificateToTrust.equals(chain[0].getIssuerX500Principal().getName())){
			storeCert(chain[0]);
			//Log.i("Cert", "Certificate Matched");
		}else {
			//Log.i("Cert", "Certificate Invalid");
		}
		
		
		
		//For Production
//		if(certificateToMatch.equals(chain[0])){
//			storeCert(chain[0]);
//			//Log.i("Cert", "Certificate Matched");
//		}else {
//			//Log.i("Cert", "Certificate Invalid");	
	...
	class MemorizingHostnameVerifier implements HostnameVerifier {
		...
		public boolean verify(String hostname, SSLSession session) {
			...
			try {
				...
				if(certificateToTrust.equals(cert.getIssuerX500Principal().getName())){
					return true;
				}
